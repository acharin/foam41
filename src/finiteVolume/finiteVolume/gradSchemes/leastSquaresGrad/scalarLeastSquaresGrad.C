...
) const
{
    ...
    forAll (nei, faceI)
    {
        ...
        // Caution - this is NOT negSumDiag(). VV, 17/July/2014.
        d[owner] -= u[faceI]*fluxIn[faceI];
        d[neighbour] -= l[faceI]*fluxIn[faceI];
    }
    ...
    forAll (vf.boundaryField(), patchI)
    {
        ...
        if (patch.coupled())
        {
            ...
            forAll (pf, faceI)
            {
                pcoupleUpper[faceI] -= cellVIn[fc[faceI]]*pownLs[faceI]*pFlux[faceI];
                pcoupleLower[faceI] -= cellVInNei[faceI]*pneiLs[faceI]*pFlux[faceI];
            }
        }
        else
        {
            ...
            forAll (pf, faceI)
            {
                ...
                d[cellI] += cellVIn[cellI]*pownLs[faceI]*internalCoeffs[faceI]*pFlux[faceI];

                source[cellI] -= cellVIn[cellI]*pownLs[faceI]*
                    boundaryCoeffs[faceI]*pFlux[faceI];
            }
        }
    }
